...
export function useCreateAdvance(operationId: string) {
  ...
  return useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.response?.data?.message ?? 'Failed to register advance');
    },
  });
}
...
export function useUpdateAdvance(operationId: string) {
  ...
  return useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.response?.data?.message ?? 'Failed to update advance');
    },
  });
}
...
export function useDeactivateAdvance(operationId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['advances', activeOrgId, operationId] });
      queryClient.invalidateQueries({ queryKey: ['finance-summary', activeOrgId, operationId] });
      toast.success('Advance deactivated');
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message ?? 'Failed to deactivate advance');
    },
  });
}
