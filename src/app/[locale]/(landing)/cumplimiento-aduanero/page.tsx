...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.cumplimientoAduanero.meta' })
...
export default async function CumplimientoAduaneroPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.cumplimientoAduanero' })
  ...
  const breadcrumb = buildBreadcrumbJsonLd([
    ...
    { name: t('breadcrumbTitle'), url: `${BASE_URL}/${locale}/cumplimiento-aduanero` },
  ])
