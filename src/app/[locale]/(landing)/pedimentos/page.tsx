...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.pedimentos.meta' })
...
export default async function PedimentosPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.pedimentos' })
  ...
  const breadcrumb = buildBreadcrumbJsonLd([
    ...
    { name: t('breadcrumbTitle'), url: `${BASE_URL}/${locale}/pedimentos` },
  ])
