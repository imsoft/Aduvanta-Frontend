...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.portalClientes.meta' })
...
export default async function PortalClientesPage({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'landing.portalClientes' })
  ...
  const breadcrumb = buildBreadcrumbJsonLd([
    ...
    { name: t('breadcrumbTitle'), url: `${BASE_URL}/${locale}/portal-clientes` },
  ])
