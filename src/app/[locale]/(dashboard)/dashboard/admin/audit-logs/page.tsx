  ...
  return (
    <div className="w-full space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Registro de actividad de toda la plataforma</p>
      </div>

      <div className="rounded-xl border bg-card overflow-hidden">
        <div className="px-5 py-3 border-b bg-muted/20 flex items-center justify-between">
          <p className="text-sm font-medium">{total.toLocaleString('es-MX')} eventos</p>
            ...
            {data?.logs.map((log) => (
              <div key={log.id} className="flex items-start gap-4 px-5 py-2.5 hover:bg-muted/20 transition-colors">
                ...
                <div className="shrink-0 text-muted-foreground whitespace-nowrap">
                  {log.createdAt ? format(new Date(log.createdAt), 'dd MMM HH:mm', { locale: es }) : '—'}
                </div>
              </div>
            ))}
            {data?.logs.length === 0 && (
              <div className="px-5 py-12 text-center text-sm text-muted-foreground font-sans">No hay eventos registrados</div>
            )}
          </div>
        )}
      </div>
